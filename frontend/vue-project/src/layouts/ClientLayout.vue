      ...
      <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
          <router-link
            ...
            :class="{ 'bg-blue-50 text-blue-600': $route.path.includes('dashboard') }"
          >
          ...
          <router-link
            to="/client/services"
            class="flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors"
            :class="{ 'bg-blue-50 text-blue-600': $route.path.includes('services') }"
          >
          ...
          <router-link
            to="/client/profile"
            class="flex items-center px-4 py-3 text-gray-700 rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-colors"
            :class="{ 'bg-blue-50 text-blue-600': $route.path.includes('profile') }"
          >
        ...
        <div class="p-4 border-t border-gray-200">
          ...
          >
            <div class="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center">
              <span class="text-blue-600 text-sm font-medium">{{ userInitials }}</span>
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900 truncate">{{ authStore.userName }}</p>
              <p class="text-sm text-gray-500 truncate">Клиент</p>
            </div>
          </router-link>
...
const currentPageTitle = computed(() => {
  ...
  const titles: Record<string, string> = {
    ...
    ClientDeals: 'Мои заявки',
    ClientServices: 'Услуги',
    ClientProfile: 'Профиль'
  }
  return titles[routeName] || 'Клиентский кабинет'
})
